...
%%  层次分析法
A=[1,1/2,1/3,1;2,1,2,3;3,1/2,1,3;1,1/3,1/3,1];
...
color=[    0.2196    0.3176    0.6392
    0.4471    0.6667    0.8078
    0.7922    0.9137    0.9529
    0.9961    0.9843    0.7294];
...
colorList=[78 101 155;
          ...
          239 164 132;
          182 118 108]./255;
th1 = linspace(2*pi/length(X)/2,2*pi-2*pi/length(X)/2,length(X));
r1 = X;
...
legend(M,Label,"FontSize",10,"LineWidth",1,'Box','off','Location','southoutside')
%%
...
hatchfill2(h(1,2),'single','HatchAngle',60,'HatchDensity',30,'HatchColor','k');
hatchfill2(h(1,3),'single','HatchAngle',0,'HatchDensity',30,'HatchColor','k');
hatchfill2(h(1,4),'single','HatchAngle',-65,'HatchDensity',30,'HatchColor','k');
